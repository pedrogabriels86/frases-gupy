...
<style>
    ...
    * { font-family: 'Inter', sans-serif; }
    ...
    /* --- CABEÇALHO INTELIGENTE (RESPONSIVO) --- */
    /* Deixa o header transparente para parecer que não existe */
    header[data-testid="stHeader"] {
        background-color: transparent !important;
        z-index: 99 !important;
    }
    
    /* Esconde a barra colorida padrão do Streamlit */
    div[data-testid="stDecoration"] { display: none; }
    
    /* Esconde os menus da direita (Deploy, 3 pontos, Github) */
    [data-testid="stHeaderActionElements"] { display: none !important; }
    .stDeployButton { display: none !important; }
    
    /* Pinta o botão do menu (hambúrguer) de azul Gupy */
    button[kind="header"] {
    ...
    /* --- AJUSTES DE LAYOUT (MOBILE VS DESKTOP) --- */
    .block-container {
        padding-top: 3rem !important; /* Espaço para o botão do menu não ficar em cima do título */
        padding-bottom: 5rem !important;
    }
    
    /* No celular, reduz as margens laterais */
    @media (max-width: 576px) {
        .block-container {
            padding-left: 1rem !important;
            padding-right: 1rem !important;
        }
        /* Ajusta tamanho dos títulos no mobile */
...
if st.session_state["usuario_logado"] is None:
    ...
    # No Desktop usa colunas para centrar. No mobile, as colunas empilham e ocupam 100%.
    ...
    with c2:
        ...
        with st.container(border=True):
            ...
            st.markdown("<h3 style='text-align:center; color:#555; font-size: 1.2rem;'>Frases de Recusa</h3>", unsafe_allow_html=True)
            with st.form("login"):
    ...
    if user.get('trocar_senha'):
        ...
        if page == "📂 Frases de Recusa":
            # Layout Flexível: No mobile, a busca vai para baixo do título automaticamente
            ...
            with c_filtro:
                st.caption(f"{len(filtrados)} resultados")
            
            for f in filtrados:
                with st.container(border=True):
                    # No Mobile, as colunas [1, 3] podem ficar apertadas.
                    # O Streamlit ajusta isso sozinho, mas vamos garantir o layout.
                    ...
                    with col_info:
                        ...
                        if f.get('revisado_por'): 
                            try:
                                ...
                                st.caption(f"✅ {f['revisado_por']} ({dt})")
                            except: pass
                    with col_cont:
        ...
        # --- 2. GESTÃO ---
            ...
            with st.expander("➕  Adicionar Nova Frase", expanded=False):
                with st.form("quick_add"):
                    c1, c2, c3 = st.columns(3) # No mobile vira uma pilha de 3 inputs
            ...
            st.write("")
            st.markdown("#### Editar e Gerenciar")
        ...
        # --- 3. ADMINISTRAÇÃO ---
                ...
                with c_list:
                    ...
                    for u in users:
                        with st.expander(f"{u['username']} {'(Admin)' if u['admin'] else ''}"):
                            ...
                            if c_x.button("Resetar Senha", key=f"r{u['id']}", use_container_width=True):
                                supabase.table("usuarios").update({"trocar_senha":True}).eq("id", u['id']).execute(); st.toast("Resetado!")
                            if u['username'] != user['username'] and c_y.button("Excluir", key=f"d{u['id']}", type="primary", use_container_width=True):
            ...
            with t2:
                ...
                if full_data: st.download_button("📥 Baixar CSV (Backup)", pd.DataFrame(full_data).to_csv(index=False).encode('utf-8'), "backup.csv", "text/csv", use_container_width=True)
                st.write("---")
