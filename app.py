...
# ==============================================================================
# 2. ESTILO CSS (ATUALIZADO PARA QUEBRA DE LINHA)
# ==============================================================================
st.markdown("""
<style>
    ...
    /* --- CSS MÁGICO PARA O TEXTO NÃO CORTAR --- */
    /* Força o st.code a quebrar linha e crescer verticalmente */
    div[data-testid="stCodeBlock"] pre {
        white-space: pre-wrap !important;   /* Quebra a linha */
        word-break: break-word !important;  /* Quebra palavras longas se necessário */
...
def card_frase(frase):
    with st.container(border=True):
        ...
        # O CSS injetado fará este componente quebrar linha automaticamente!
...
def tela_biblioteca(user):
    ...
    if not filtros_ativos: st.caption("🔥 Destaques: Mostrando as 4 frases mais recentes.")
    else: st.caption(f"🔎 Resultados encontrados: {len(dados)}")
    st.divider()
            ...
            with st.form("form_add", clear_on_submit=True):
                ...
                if st.form_submit_button("💾 Salvar Frase", type="primary", use_container_width=True):
                    try:
                        ...
                        if check.data: st.warning("Atenção: Já existe uma frase com este conteúdo exato.")
                        else:
        ...
        if arquivo:
            if st.button("🚀 Processar e Importar", type="primary"):
                try:
                    ...
                    if not required_cols.issubset(df.columns): st.error(f"Erro: Faltam colunas: {', '.join(required_cols)}")
                    else:
                        ...
                        for i, row in df.iterrows():
                            try:
                                ...
                                if ass_atual in assinaturas_existentes: duplicados += 1
                                    ...
                                    user_excel = str(row.get('usuario')).strip() if pd.notnull(row.get('usuario')) and str(row.get('usuario')).strip() else user['username']
                                    data_excel = pd.to_datetime(row.get('data')).strftime('%Y-%m-%d') if pd.notnull(row.get('data')) else datetime.now().strftime('%Y-%m-%d')
                                    supabase.table("frases").insert({"empresa": el, "documento": padronizar(row.get('documento', 'Geral')), "motivo": ml, "conteudo": cl, "revisado_por": user_excel, "data_revisao": data_excel}).execute()
                                    assinaturas_existentes.add(ass_atual); sucesso += 1
                            except: erros += 1
                            progress.progress((i + 1) / total)
                        st.success("Concluído!"); c1, c2, c3 = st.columns(3); c1.metric("✅", sucesso); c2.metric("🚫", duplicados); c3.metric("⚠️", erros)
                        if sucesso > 0: registrar_log(user['username'], "Importação em Massa", f"Importou {sucesso} itens."); time.sleep(4); st.cache_data.clear(); st.rerun()
...
def tela_manutencao(user):
    ...
    else: query = query.limit(4); st.caption("4 últimas adições.")
    ...
    for item in items:
        with st.expander(f"#{item['id']} | {item['empresa']} - {item['motivo']}"):
            with st.form(key=f"form_edit_{item['id']}"):
                c1, c2, c3 = st.columns([1.5, 1.5, 1]); ne = c1.text_input("Empresa", value=item['empresa']); nm = c2.text_input("Motivo", value=item['motivo']); nd = c3.text_input("Doc", value=item['documento']); nc = st.text_area("Conteúdo", value=item['conteudo'], height=150)
                st.write("---"); cs, cd = st.columns([1, 0.25])
                ...
                if cd.form_submit_button("🗑️ Excluir"):
                    supabase.table("frases").delete().eq("id", item['id']).execute(); registrar_log(user['username'], "Excluir Frase", f"ID: {item['id']}"); st.toast("Excluído!"); time.sleep(1); st.cache_data.clear(); st.rerun()
...
def tela_admin(user_logado):
    ...
    with tab_users:
        ...
        for u in users:
            with st.container(border=True):
                c1, c2 = st.columns([3, 1]); c1.write(f"👤 **{u['username']}** {'(Admin)' if u.get('admin') else ''}")
                with st.expander("Editar"):
                    with st.form(key=f"eu_{u['id']}"):
                        np = st.text_input("Senha", value=u['senha'], type="password"); ia = st.checkbox("Admin", value=u.get('admin', False))
                        if st.form_submit_button("Atualizar"): supabase.table("usuarios").update({"senha": np, "admin": ia}).eq("id", u['id']).execute(); registrar_log(user_logado['username'], "Editar Usuário", u['username']); st.rerun()
                        if st.form_submit_button("Excluir"):
                            if u['username'] == user_logado['username']: st.error("Erro: Auto-exclusão.")
                            else: supabase.table("usuarios").delete().eq("id", u['id']).execute(); registrar_log(user_logado['username'], "Excluir Usuário", u['username']); st.rerun()
        with st.expander("➕ Novo Usuário"):
            with st.form("nu"):
                nu = st.text_input("User"); ns = st.text_input("Pass", type="password"); na = st.checkbox("Admin")
                if st.form_submit_button("Criar"):
                    try: supabase.table("usuarios").insert({"username": nu, "senha": ns, "admin": na}).execute(); st.success("Criado!"); st.rerun()
                    except: st.error("Erro.")
    with tab_logs:
        if st.button("Atualizar Logs"): st.rerun()
        try: logs = supabase.table("logs").select("*").order("id", desc=True).limit(50).execute().data; st.dataframe(logs, use_container_width=True, hide_index=True)
        except: st.error("Sem logs.")
    with tab_backup:
        c1, c2 = st.columns(2)
        with c1: df = supabase.table("frases").select("*").execute().data; st.download_button("⬇️ Backup Frases", data=converter_para_csv(df), file_name="frases.csv", mime="text/csv", use_container_width=True)
        with c2: du = supabase.table("usuarios").select("*").execute().data; st.download_button("⬇️ Backup Usuários", data=converter_para_csv(du), file_name="users.csv", mime="text/csv", use_container_width=True)
    with tab_danger:
        ...
        with col_danger1:
            ...
            if st.button("💥 APAGAR TODAS AS FRASES", type="primary", disabled=st.text_input("Digite 'CONFIRMAR':", key="cp")!="CONFIRMAR", use_container_width=True):
                supabase.table("frases").delete().neq("id", 0).execute(); registrar_log(user_logado['username'], "RESET FRASES", "Apagou todas as frases"); st.success("Removido."); time.sleep(2); st.cache_data.clear(); st.rerun()
        with col_danger2:
            st.markdown("#### 🏭 Reset Usuários"); st.caption("Remove outros usuários.")
            if st.button("💥 APAGAR OUTROS USUÁRIOS", type="primary", disabled=st.text_input("Digite 'RESETAR TUDO':", key="cu")!="RESETAR TUDO", use_container_width=True):
                supabase.table("usuarios").delete().neq("username", user_logado['username']).execute(); registrar_log(user_logado['username'], "RESET USUARIOS", "Apagou outros usuários"); st.success("Removido."); time.sleep(2); st.rerun()
    ...
    with c2:
        ...
        with st.container(border=True):
            ...
            if st.button("Entrar", type="primary", use_container_width=True):
                ...
                if user: st.session_state["usuario_logado"] = user; cookie_manager.set("gupy_token", u, expires_at=datetime.now() + timedelta(days=7)); st.rerun()
